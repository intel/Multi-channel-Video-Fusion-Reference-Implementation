...
PlayerWindow::PlayerWindow(QWidget *parent) : QWidget(parent)
{
    ...
    vl->addLayout(hb);
    m_cboxUrl = new QComboBox(this);

    urls << "rtmp://127.0.0.1:1935/live";
    urls << "C:/work/ffmpeg_src/linux_video/backgroud_1080.mp4";
    m_cboxUrl->addItems(urls);
    m_cboxUrl->setCurrentIndex(0);
    m_cboxUrl->setEditable(true);

    m_openBtn = new QPushButton(tr("Open"));
    m_playBtn = new QPushButton(tr("Play/Pause"));
    m_stopBtn = new QPushButton(tr("Stop"));
    hb->addWidget(m_cboxUrl);
...
void PlayerWindow::openMedia()
{
    ...
    QString file = m_cboxUrl->currentText().trimmed() ;

    //QString file = "rtmp://127.0.0.1:1935/live";
    if (file.isEmpty())
        return;
    m_player->play(file);
}
...
void PlayerWindow::seekBySlider(int value)
{
    if (!m_player->isPlaying())
        return;
    m_player->seek(qint64(value*qint64(m_unit)));
}
